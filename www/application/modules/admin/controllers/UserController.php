...
class Admin_UserController extends Zend_Controller_Action {
    ...
    var $actions = array();
    var $options = array();
    var $singular = "Administrador";
    var $plural = "todos los Administradores";
    var $messageDelete = "¿Esta seguro que desea eliminar este Administrador?";
    var $title = "Configuración / Administradores";
...
//MODELS      
    ...
    public function indexAction() {
        try {
            ...
            $results_all = $this->user->showAll($where, $this->view->parameters['sort'], $this->view->parameters['order']);
    ...
    public function editAction() {
        try {
            if ($this->getRequest()->isPost()) {
                ...
                $this->user->edit($this->data, $this->view->parameters["id"]);
                $this->_helper->flashMessenger->addMessage(array('type' => 'success', 'message' => MESSAGE_EDI));
                $this->_helper->Redirector->gotoSimple('index', null, null);
            }
            $this->view->result = $this->user->get($this->view->parameters["id"]);
    ...
    public function deleteAction() {
        try {
            ...
            if ($this->getRequest()->isPost()) {
                if ($this->view->loginInfo["ad_id"] == $this->getRequest()->getPost('id')):
                    throw new Zend_Controller_Action_Exception("No se puede eliminar así mismo.");
                endif;
                $this->user->delete_row($this->getRequest()->getPost('id'));
            }
        } catch (Zend_Exception $exc) {
            exit($exc->getMessage());
        }
    ...
    public function detailAction() {
        try {
            ...
            $this->view->description = 'Detalle de ' . $this->title . ' "' . $this->view->result[$this->view->fields[5]["field"]] . '"';
            if ($this->getRequest()->isXmlHttpRequest()):
