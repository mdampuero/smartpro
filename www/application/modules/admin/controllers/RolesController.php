...
class Admin_RolesController extends Zend_Controller_Action {
    ...
    var $actions = array();
    var $options = array();
    var $singular = "Rol de Administrador";
    var $plural = "todos los Roles de Administrador";
    var $messageDelete = "¿Esta seguro que desea eliminar este Rol de Administrador?";
    var $title = "Configuración / Roles de Administrador";
    ...
    public function init() {
        try {
            ...
            $this->fields = array(
                ...
                array('field' => 'ar_name', 'label' => 'Nombre', 'list-edit' => false, 'required' => 'required', 'list' => true, 'search' => true, 'order' => true),
            );
    ...
    public function indexAction() {
        try {
            ...
            $paginator->setItemCountPerPage(COUNTPERUGGE)
                    ->setCurrentPageNumber($this->_getParam('page', 1))
                    ->setPageRange(UGGERANGE);
    ...
    public function editAction() {
        try {
            if ($this->getRequest()->isPost()) {
                $this->data = $this->_helper->Form->isValid($this->fields);
                $id = $this->model->edit($this->data, $this->view->parameters["id"]);
                $this->_helper->flashMessenger->addMessage(array('type' => 'success', 'message' => MESSAGE_EDI));
                $this->_helper->Redirector->gotoSimple('index', null, null);
            }
    ...
    public function deleteAction() {
        try {
            ...
            if ($this->getRequest()->isPost()) {
                $this->model->delete_row($this->view->parameters["id"]);
            }
        } catch (Zend_Exception $exc) {
            exit($exc->getMessage());
        }
